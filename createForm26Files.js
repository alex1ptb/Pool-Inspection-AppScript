...
function createForm26Files() {
  ...
  rows.forEach((row,index) => {
    ...
    if (row[headerKeys[0].indexOf("Link to Generated Form 26 for customer")+1]) {
      return
      };
    // if business is AA, skip the form process
    if (row[businessColumnNum].valueOf() == "AA") { Logger.log('found AA');return}
    //create copy of template and put in destination folder
    const copy = form26Template.makeCopy(`${row[headerKeys[0].indexOf("Name")]} ${row[headerKeys[0].indexOf("Timestamp")]}`, destinationFolderForCreatedDocs)
    //open doc
    ...
    if (row[codeViolationsColumnNum].valueOf().toString().indexOf(' ') != -1) {
      var codeViolationsToLookUp = row[codeViolationsColumnNum].valueOf().split(' ');
    ...
    body.replaceText('{{postcode}}',row[headerKeys[0].indexOf("Postcode")]);
    body.replaceText('{{lotdetails}}',row[headerKeys[0].indexOf("Lot Number")]);
    body.replaceText('{{plandetails}}',row[headerKeys[0].indexOf("CH/RP/SP/GTP/BUP")]);
    body.replaceText('{{governmentarea}}',row[headerKeys[0].indexOf("Local Government Area")]);
    body.replaceText('{{inspectionvideo}}',row[headerKeys[0].indexOf("Inspection Video Link")]);
    body.replaceText('{{codeviolations}}',codeViolationInfo);
    body.replaceText('{{inspectiondate}}',new Date(row[headerKeys[0].indexOf("Date of Inspection")])
      .toLocaleDateString('pt-PT'));
    body.replaceText('{{date}}', new Date().toLocaleDateString('pt-PT'));
    if (row[headerKeys[0].indexOf("Type of Fence")].valueOf() == "NOT Shared"){
    ...
    sheet.getRange(index + 3,headerKeys[0].indexOf("Link to Generated Form 26 for customer")+1).setValue(pdfURL)
  }}})
}