  ...
  for (let i = 0; i < arrayOfRows.length; i++) {
    for (let k = 0; k < checkedBoxes.length; k++) {
      if (arrayOfRows[i]["ID"] == checkedBoxes[k][1]) {
        ...
        if (
          formResponse
            .getRange(2 + i, headerKeys[0].indexOf("Code Violations") + 1)
        ...
        if (
          ...
          if (
            formResponse.getRange(2 + i, attachmentURL).getDisplayValue() == ""
          ) {
            createForm26Files();
          }
          ...
          if (
            ...
            var fileUrl =
              arrayOfRows[i]["Link to Generated Form 26 for customer"];
            if (fileUrl == "") {
              createForm26Files();
            }
            ...
            function getIdFromUrl(fileUrl) {
              return fileUrl.match(/[-\w]{25,}(?!.*[-\w]{25,})/);
            }
